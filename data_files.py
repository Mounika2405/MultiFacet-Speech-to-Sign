...
class SignProdDataset(data.Dataset):
    ...
    def __init__(self, path, exts, fields, trg_size, src_size, num_sec, skip_frames=1, is_train=True, **kwargs):
        ...
        with h5py.File(src_path, 'r') as src_file, \
            ...
            src_group = src_file[split]
            trg_group = trg_file[split]
            # For Source, Target and FilePath
            for src_line, trg_line, nonreg_line, files_line in zip(src_group.keys(), trg_group.keys(), nonreg_trg_file, files_file):
                i+= 1
                ...
                src_data = src_group[src_line][()].T
                trg_data = trg_group[trg_line][()]
                
                # # Split target into joint coordinate values
                # trg_line = trg_line.split(" ")
                # if len(trg_line) == 1:
                #     continue
                # Turn each joint into a floapyt value, with 1e-8 for numerical stability
                ...
                # total_keypts = 85
                # total_vars = (total_keypts * 3) + 1
                body_only_keypts = 48
                body_vars = body_only_keypts * 3
                trg_frames = trg_data[list(range(0, trg_data.shape[0], skip_frames))]
                trg_frames = [trg_line[:body_vars].tolist() + [trg_line[-1]] for trg_line in trg_frames]
