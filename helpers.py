...
# Find the best timing match between a reference and a hypothesis, using DTW
def calculate_dtw(references, hypotheses, max_feature_index=None):
    """
    Calculate the DTW costs between a list of references and hypotheses using SoftDTW (approx)
    ...
    euclidean_norm = lambda x, y: torch.sum(torch.abs(x - y))
    ...
    for i, ref in enumerate(references):
        ...
        ref_count = ref[:ref_max_idx,:-1]
        if max_feature_index is not None:
            ref_count = ref_count[:, :max_feature_index + 1]  # Useful if we want to use only certain points for dtw
        ...
        if max_feature_index is not None:
            hyp_count = hyp_count[:, :max_feature_index + 1] # Useful if we want to use only certain points for dtw
...
def calculate_pck(pred, gt, alpha=0.2):
    '''
    ...
    pred = pred.reshape((batch_size,-1,3)).transpose(0,2,1)

    num_keypts = pred.shape[2]
    
    gt = gt.reshape((batch_size,num_keypts,3)).transpose(0,2,1)
    

    pck_radius = compute_pck_radius(gt, alpha, num_keypts)
...
def compute_pck_radius(gt, alpha, num_keypts):
    ...
    max_axis_per_keypoint = np.tile(np.expand_dims(max_axis, -1), [1, num_keypts])
    return max_axis_per_keypoint * alpha


def save_hyp_keypoints(out_dir, file_paths, hypotheses, references=None):
    for idx, fp in enumerate(file_paths):
        output_fp = os.path.join(out_dir, fp)
        os.makedirs(os.path.dirname(output_fp), exist_ok=True)
        np.save(output_fp + '_hyp.npy', hypotheses[idx][1:].cpu().numpy())
        if references is not None:
            np.save(output_fp + '_ref.npy', references[idx].cpu().numpy())
